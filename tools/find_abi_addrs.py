...
XEX_PATH = os.path.join(os.path.dirname(__file__), "..", "extracted", "default.xex")
PE_PATH  = os.path.join(os.path.dirname(__file__), "..", "extracted", "default_pe.exe")
EXTRACT_EXE = None  # Not needed, PE is pre-extracted
...
def main():
    # Allow PE path as command-line argument
    pe_path = sys.argv[1] if len(sys.argv) > 1 else PE_PATH

    # Ensure PE is extracted
    if not os.path.exists(pe_path):
        print("PE binary not found at %s" % pe_path)
        if EXTRACT_EXE and os.path.exists(EXTRACT_EXE):
            ...
            print("ERROR: PE not found. Extract it first with extract_pe.py")
            return
